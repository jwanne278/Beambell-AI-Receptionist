...
class SpeechToText {
    constructor() {
        ...
        this.mic = new Microphone({
            rate: 11000, // Higher sample rate for better voice quality
    ...
    async start() {
        try {
            ...
            this.connection = this.deepgram.listen.live({
                encoding: 'mulaw', // Keep mu-law encoding
                sample_rate: 11000, // Higher sample rate for better voice quality
                channels: 1, // Mono audio
                ...
                endpointing: 500, // Reduced for faster response while still natural
                utterance_end_ms: 1000 // Keep standard utterance end time
            });

            // Handle successful connection to Deepgram
            this.connection.on(LiveTranscriptionEvents.Open, () => {
                console.log('Connection to Deepgram established'.green);
                console.log('Starting microphone (Balanced Mode: 11kHz with Nova-3)...'.yellow);
                
                // Start recording from microphone
                this.micStream = this.mic.startRecording();
                
                // Send microphone data to Deepgram when received
                this.micStream.on('data', async (data) => {
                    if (this.connection && this.connection.getReadyState() === 1) {
                        try {
                            // Isolate voice from the audio before sending to Deepgram
                            const isolatedData = await isolateVoice(data);
                            this.connection.send(isolatedData);
                        } catch (error) {
                            console.error('Voice isolation error:'.red, error);
                            // Fall back to sending original audio if isolation fails
